...
# Passcode security (store a hashed passcode)
PASSCODE_HASH = hashlib.sha256("232345".encode()).hexdigest()
...
# Decrypt the file using AES and RSA for the AES key with passcode verification
def decrypt_file(file_path, passcode):
    if hashlib.sha256(passcode.encode()).hexdigest() != PASSCODE_HASH:
...
def decrypt():
    ...
    if not file or not passcode:
        return "File and passcode required", 400
    ...
    try:
        decrypted_file_path = decrypt_file(file_path, passcode)
